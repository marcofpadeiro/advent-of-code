...
func main() {
	...
	for i, galaxy := range galaxies {
		for j, pair := range galaxies {
			if i > j {
				//println(galaxy.x, galaxy.y, pair.x, pair.y, find_distance(galaxy, pair, empty_cols, empty_lines))
...
func find_distance(pair1, pair2 Coordinates, empty_cols, empty_lines []int) int {
	...
	distanceX := xMax - xMin - emptyColsBetween + (emptyColsBetween * 2)
	distanceY := yMax - yMin - emptyLinesBetween + (emptyLinesBetween * 2)
